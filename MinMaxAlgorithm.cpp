...
void MinMaxAlgorithm::DevelopChildren() {
	...
	GraphNode<u_int8_t> *graphNodeChild;
	...
	for (short column = 0; column < FOUR_IN_ROW_PUZZLE_WIDTH; column++) {
		...
		if (!this->aPuzzle.IsColumnFull(column)) {
			...
			graphNodeChild = new GraphNode<u_int8_t>;
...
short MinMaxAlgorithm::GetMaxChildValue(GraphNode<u_int8_t> *&aGraphNode) {
	short max = NOT_ASSIGNED;
	
	for (typename vector< GraphNode<u_int8_t>* >::iterator it = aGraphNode->Edges->begin(); it != aGraphNode->Edges->end(); it++)
...
short MinMaxAlgorithm::GetMinChildValue(GraphNode<u_int8_t> *&aGraphNode) {
	short min = NOT_ASSIGNED;
